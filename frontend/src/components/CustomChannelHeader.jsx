...
const CustomChannelHeader = () => {
  ...
  const otherUser = Object.values(channel.state.members).find(
    //getting other user's info to display on the header, or if channel, then channel's info like name,profile img
    (member) => member.user.id !== user.id,
  );

  const isDM =
    channel.data?.member_count === 2 && channel.data?.id.includes("user_"); //if it consists of a user_, then it's a direct message.
